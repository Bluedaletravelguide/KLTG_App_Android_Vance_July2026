...
class _NL_Tab extends StatelessWidget {
  final String category;
  final HomePalette palette;
  const _NL_Tab({required this.category, required this.palette});
  @override
  Widget build(BuildContext context) {
    return ApiFutureView<List<Map<String, dynamic>>>(
      load: () => fetchList('appExploreKL_NL', category: category),
      isEmpty: (data) => data.isEmpty,
      builder: (context, data) {
        final items = data.map((e) => ItemData3(field(e, 'title'), field(e, 'image'), field(e, 'content'), field(e, 'location'), field(e, 'hours'), field(e, 'phone'))).toList();
        return MyList3(items: items, palette: palette);
      },
    );
  }
}
      ...
      child: Scaffold(
          ...
          bottom: PreferredSize(
            ...
            child: Container(
              ...
              child: TabBar(
                ...
                indicatorWeight: 3,
                tabs: [Tab(text: S.of(context).nightLife), Tab(text: S.of(context).bars), Tab(text: S.of(context).nightMarket)],
        ...
        body: Container(
          ...
          child: TabBarView(children: [
            _NL_Tab(category: 'nightlife', palette: palette),
            _NL_Tab(category: 'bars', palette: palette),
            _NL_Tab(category: 'nightmarket', palette: palette),
          ]),
        ),
      ),
    );
...
class MyList3 extends StatelessWidget {
  ...
  const MyList3({super.key, required this.items, required this.palette});
