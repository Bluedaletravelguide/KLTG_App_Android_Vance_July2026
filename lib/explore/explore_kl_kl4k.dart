  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ApiFutureView<List<Map<String, dynamic>>>(
        load: () => fetchList('appExploreKL_KL4K'),
        isEmpty: (data) => data.isEmpty,
        builder: (context, data) {
          final items = data.map((e) => ItemData6(field(e,'title'),field(e,'image'),field(e,'location'),field(e,'hours'),field(e,'phone'))).toList();
          return MyList6(items: items, palette: palette);
        },
      ),
    );
  }
...
class MyList6 extends StatelessWidget {
  ...
  const MyList6({super.key, required this.items, required this.palette});
